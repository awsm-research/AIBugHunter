...
from compact_bert_model import CompactBERT
import pandas as pd
# metrics
...
def train(args, train_dataset, model, tokenizer, eval_dataset, cwe_label_map):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            (inputs_ids, cwe_type_labels, cwe_id_labels) = [x.to(args.device) for x in batch]
            model.train()
            cwe_id_loss, cwe_type_loss, contrastive_loss_1, contrastive_loss_2 = model(input_ids=inputs_ids, 
                                               cwe_type_labels=cwe_type_labels,
                                               cwe_id_labels=cwe_id_labels,
                                               cwe_label_map=cwe_label_map)
            if args.n_gpu > 1:
                cwe_id_loss = cwe_id_loss.mean()
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False, cwe_label_map=None):
    ...
    for batch in eval_dataloader:
        (inputs_ids, cwe_type_labels, cwe_id_labels) = [x.to(args.device) for x in batch]
        with torch.no_grad():
            cwe_id_prob, cwe_type_prob = model(input_ids=inputs_ids, cwe_label_map=cwe_label_map)
...
def test(args, model, test_dataset, cwe_label_map=None, type_label_map=None):
    # build dataloader
    ...
    test_df.to_csv("./raw_predictions/multitask_codebert_raw_predictions.csv", index=False)
...
def main():
    ...
    model = CompactBERT(roberta=roberta, 
                        tokenizer=tokenizer, 
    ...
    if args.do_test:
        ...
        test(args, model, test_dataset, cwe_label_map=cwe_label_map, type_label_map=type_label_map)
    return results
