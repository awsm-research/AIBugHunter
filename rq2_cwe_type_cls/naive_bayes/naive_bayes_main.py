...
# load train, val data
train = pd.read_csv('../data/train_wt_type.csv')
val = pd.read_csv('../data/val_wt_type.csv')
# use train + val data to fit the model
train_data = pd.concat([train, val])
# load test data
test_data = pd.read_csv('../data/test_wt_type.csv')
# textual code data
X_train = train_data["func_before"]
X_test = test_data["func_before"]
# labels
y_train = train_data["type_label"].tolist()
for i in range(len(y_train)):
    # transform to one-hot labels
    y_train[i] = type_label_map[y_train[i]][0]

y_test = test_data["type_label"].tolist()
for i in range(len(y_test)):
    # transform to one-hot labels
    y_test[i] = type_label_map[y_test[i]][0]
